...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, scale=None, attn_mask=None, bias=None):
        ...
        if bias != None:
            attention = bias[0] * attention# + attention
            attention = bias[1] + attention
            #attention = attention
            
        if attn_mask is not None:
            assert attn_mask.size() == attention.size()
            attention.masked_fill_(attn_mask == 0, -1e9)
                # 计算softmax
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, key, value, query, attn_mask=None, bias=None):
        ...
        if bias != None and len(bias) >= 3:
            attn_mask = bias[2].unsqueeze(1).repeat(1,num_heads, 1, 1)
...
class LayerNorm(nn.Module):
    ...
    def __init__(self, features, epsilon=1e-12):
        ...
        self.pre_scale = torch.nn.Parameter(torch.FloatTensor(self._batch_size, 12, self._emb_size), requires_grad=True)
        self.pre_bias = torch.nn.Parameter(torch.FloatTensor(self._batch_size, 12, self._emb_size), requires_grad=True)
        self.weight_init()
    def weight_init(self):
    ...
    def forward(self, src):
        ...
        norm_x = norm_x*self.pre_scale
        norm_x = norm_x+self.pre_bias
        return norm_x
